...
pub async fn call(name: &str, data: &[Value]) -> Result<Value> {
    ...
    let text = res.text().await.map_err(|e| {
        logging!(error, Type::Engine, true, "aria2c read error: {}", e);
        e
    })?;
    ...
    if !status.is_success() {
        ...
        logging!(
            ...
            Type::Engine,
            true,
            "aria2c http error: {} body: {}",
            status,
            preview
        );
    }

    let res: Value = match serde_json::from_str(&text) {
        Ok(value) => value,
        Err(err) => {
            let preview: String = text.chars().take(300).collect();
            logging!(
                ...
                Type::Engine,
                true,
                "aria2c decode error: {} body: {}",
                err,
                preview
            );
            return Err(anyhow::anyhow!("aria2c decode error: {}", err));
        }
    };
