...
impl IAria2Temp {
    pub fn new() -> Self {
        ...
        if normalize_macos_x64_limits(&mut map) {
            is_changed = true;
        }
    ...
    pub fn template() -> Self {
        ...
        map.insert(
            ...
            default_max_connection_per_server().to_string(),
        );
...
fn is_macos_x64() -> bool {
    cfg!(target_os = "macos") && std::env::consts::ARCH == "x86_64"
}
...
fn normalize_macos_x64_limits(map: &mut HashMap<String, String>) -> bool {
    if !is_macos_x64() {
        return false;
    }
    ...
    if let Some(value) = map.get("max-connection-per-server") {
        if let Ok(parsed) = value.parse::<u32>() {
            if parsed > max_value {
                map.insert("max-connection-per-server".into(), max_value.to_string());
                changed = true;
            }
        }
    }
